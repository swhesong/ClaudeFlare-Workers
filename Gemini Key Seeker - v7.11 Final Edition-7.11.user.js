...
// @license      MIT
// ==/UserScript==
console.log('%c HELLO! Key Seeker Script is trying to run! ', 'background: #222; color: #bada55; font-size: 20px;');
    ...
    function initialize() {
        ...
        document.head.appendChild(style);
            ...
            try {
             ...
             log("document.body not found, will retry.");
        }
    }

    // --- [最终解决方案] 脚本启动入口，使用 MutationObserver 智能等待页面加载 ---
    // 这个观察器会监视页面的变化。
    const observer = new MutationObserver((mutations, obs) => {
        // 我们寻找一个在页面最终加载完成后肯定会存在的稳定元素。
        // 这里的 "Logout" 按钮就是一个很好的目标。
        const logoutButton = document.querySelector('a[href*="logout"]');
        // 如果找到了“Logout”按钮，并且我们的脚本按钮还不存在
        if (logoutButton && !document.getElementById('key-seeker-btn')) {
            log("Stable element found. Page is ready. Injecting button now.");
            initialize(); // 调用我们原来的注入函数
            // 任务完成，停止观察，避免不必要的性能开销
            obs.disconnect();
        }
    });
    // 开始观察整个文档的变化，特别是子元素的添加或删除。
