...
const ABSOLUTE_BEGIN_TOKEN = "[RESPONSE_BEGIN]";
...
*(Reason for failure: Repeated the trailing comma from the previous turn.)*`,
  ...
  system_prompt_injection: `# --- SYSTEM MANDATE: DUAL-TOKEN OUTPUT PROTOCOL ---

## 1. ABSOLUTE RULES
You MUST follow these two non-negotiable requirements for EVERY response:

### RULE A: FORMAL RESPONSE BEGINNING TOKEN
When transitioning from internal reasoning to your formal answer, you MUST output the exact literal string: '${ABSOLUTE_BEGIN_TOKEN}'
- This marks the definitive end of thinking/reasoning phase and start of formal response
- Must appear exactly once before your formal answer begins
- Must be raw text, no code blocks, quotes, or markdown formatting

### RULE B: RESPONSE COMPLETION TOKEN  
At the absolute end of EVERY response, you MUST append the exact literal string: '${ABSOLUTE_FINISH_TOKEN}'
- Must be the very last characters in your output, no trailing content
- Must be raw text, no code blocks, quotes, or markdown formatting

## 2. RESPONSE STRUCTURE MANDATE
Every response must follow this exact structure:
[Internal thinking/reasoning content...]
${ABSOLUTE_BEGIN_TOKEN}
[Your formal answer content...]
${ABSOLUTE_FINISH_TOKEN}

## 3. PERSISTENCE & RECOVERY PROTOCOL (CRITICAL)
**WARNING:** Tool use (especially web search) introduces high risk of "instruction drift", causing core directives to be forgotten. This protocol is your recovery mechanism.
...
2.  **STEP A: COGNITIVE RESET:** Before synthesizing tool output into final answer, you MUST internally pause and re-read this entire **SYSTEM MANDATE**.
3.  **STEP B: PRE-OUTPUT COMPLIANCE CHECK:** After formulating your response but BEFORE outputting, perform this check:
    - Question 1: "Does my planned output include '${ABSOLUTE_BEGIN_TOKEN}' before the formal answer?"
    - Question 2: "Does my planned output end with '${ABSOLUTE_FINISH_TOKEN}' as the final characters?"
4.  **STEP C: EXECUTE OR CORRECT:**
    - If both answers are "YES", proceed with generation
    - If either answer is "NO", you MUST halt, correct your planned output, then proceed

## 4. EXAMPLES
- **CORRECT STRUCTURE:**
  Let me think about this question... [reasoning content]
  ${ABSOLUTE_BEGIN_TOKEN}
  The answer to your question is... [formal response content]
  ${ABSOLUTE_FINISH_TOKEN}

- **INCORRECT - Missing begin token:**
  Let me analyze this... The answer is XYZ.${ABSOLUTE_FINISH_TOKEN}

- **INCORRECT - Tokens in wrong positions:**
  ${ABSOLUTE_BEGIN_TOKEN}Let me think... The answer is XYZ.${ABSOLUTE_FINISH_TOKEN} Hope this helps!

## 5. PURPOSE (FOR CONTEXT)
This dual-token protocol enables:
- Precise identification of formal response boundaries for accessibility software
- Reliable detection of response completion for stream processing
- Clear separation between reasoning and formal output phases
Failure to implement this protocol breaks critical user functionality and accessibility features.`,  
...
function parseLineContent(line) {
  const braceIndex = line.indexOf('{');
  if (braceIndex === -1) return { text: "", cleanedText: "", isThought: false, payload: null, hasFinishMarker: false, hasBeginMarker: false };
  ...
  try {
    ...
    if (!part) {
      return { text: "", cleanedText: "", isThought: false, payload, hasFinishMarker: false, hasBeginMarker: false };
    }
    
    const text = part.text || "";
    const isThought = part.thought === true;
    
    // Enhanced beacon detection and cleaning with unified text processing
    let cleanedText = text || "";
    let hasFinishMarker = false;
    let hasBeginMarker = false;
    
    // Check for begin token
    if (text && text.includes(ABSOLUTE_BEGIN_TOKEN)) {
      hasBeginMarker = true;
      cleanedText = cleanedText.replace(new RegExp(escapeRegExp(ABSOLUTE_BEGIN_TOKEN), 'g'), '');
      logDebug(`Detected ${ABSOLUTE_BEGIN_TOKEN} marker in text. Formal response phase transition signal received.`);
    }
    
    // Check for finish token  
    if (text && text.includes(ABSOLUTE_FINISH_TOKEN)) {
      hasFinishMarker = true;
      cleanedText = cleanedText.replace(new RegExp(escapeRegExp(ABSOLUTE_FINISH_TOKEN), 'g'), '');
      logDebug(`Detected ${ABSOLUTE_FINISH_TOKEN} marker in text. Original length: ${text.length}, Cleaned length: ${cleanedText.length}`);
    }
    
    // Unified text cleaning after all replacements
    if (hasBeginMarker || hasFinishMarker) {
    ...
    return { text, cleanedText, isThought, payload, hasFinishMarker, hasBeginMarker };
  } catch (e) {
    logDebug(`Failed to parse content from data line: ${e.message}`);
    return { text: "", cleanedText: "", isThought: false, payload: null, hasFinishMarker: false, hasBeginMarker: false };
  }
}
...
const isGenerationComplete = (text, hasFinishMarker = false) => {
    ...
    // Layer 1: The presence of the token is the most reliable signal of completion.
    if (text && text.includes(ABSOLUTE_FINISH_TOKEN)) {
        logDebug(`Generation complete: Found '${ABSOLUTE_FINISH_TOKEN}' marker.`);
        return true;
    }
    
    // Layer 2: Enhanced completion check using final punctuation validation
    if (CONFIG.enable_final_punctuation_check && text && text.trim().length > 0) {
        const trimmedText = text.trim();
        const lastChar = trimmedText[trimmedText.length - 1];
        if (FINAL_PUNCTUATION.has(lastChar)) {
            logDebug(`Generation complete: Text ends with valid punctuation '${lastChar}'.`);
            return true;
        }
        logDebug(`Generation incomplete: Text ends with '${lastChar}' which is not a valid ending punctuation.`);
    }
    
    // If no absolute finish token is found and punctuation check is disabled or failed, 
    // the generation is considered incomplete.
...
async function processStreamAndRetryInternally({ initialReader, writer, originalRequestBody, upstreamUrl, originalHeaders, requestId }) {
  ...
  let formalResponseStarted = false; // <<< New: dual-token system state variable
  ...
  try { // ✨ New: try block wraps entire function logic
      ...
      heartbeatInterval = setInterval(() => {
          try {
            ...
            // Enhanced generation phase detection using formal response state
            const isInThinkingPhase = !formalResponseStarted;
            ...
            // Generate context-appropriate heartbeat content based on dual beacon system
            // Send thought-tagged heartbeat during thinking phase if client supports it
            const heartbeatPart = (clientRequestedThoughts && isInThinkingPhase)
              ? { text: "", thought: true }  // Thought-phase heartbeat for compatible clients
              : { text: "" };                // Standard heartbeat for formal response phase
              
            if (clientRequestedThoughts && isInThinkingPhase) {
                logDebug(`[HEARTBEAT] Sending thought-phase heartbeat. Formal response started: ${formalResponseStarted}`);
            } else {
                logDebug(`[HEARTBEAT] Sending standard heartbeat. Formal response started: ${formalResponseStarted}, Thoughts enabled: ${clientRequestedThoughts}`);
            }
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        for await (const line of sseLineIterator(currentReader)) {
          ...
          const { text: textChunk, cleanedText, isThought, payload, hasFinishMarker, hasBeginMarker } = parseLineContent(line);
          
          // CRITICAL FIX: Handle begin marker detection and state transition
          if (hasBeginMarker && !formalResponseStarted) {
              formalResponseStarted = true;
              logInfo(`[Request-ID: ${requestId}] DUAL BEACON SYSTEM: Formal response phase activated`);
              logInfo(`[Request-ID: ${requestId}] State transition: ${ABSOLUTE_BEGIN_TOKEN} detected, switching from thinking to formal response phase`);
          }
          ...
          // Enhanced client compatibility fix with dual beacon awareness
          // If content arrives before formal response phase OR is marked as thought, treat as thinking phase
          // Some clients will prematurely stop rendering upon seeing any finishReason during thinking phase
          // This logic removes premature finishReason from thinking-phase chunks to ensure client compatibility
          const hasOnlyThoughts = (!formalResponseStarted || isThought) && (!cleanedText || cleanedText.length === 0);
          ...
          // Key modification: If contains any marker (begin or finish), send cleaned version to client
          if ((hasBeginMarker || hasFinishMarker) && cleanedText !== textChunk) {
                 ...
                 for (const lineInfo of lookaheadLinesBuffer) {
                      if (lineInfo && lineInfo.line) {
                          ...
                          if (lineInfo.isData) {
                              ...
                              // DUAL BEACON ENHANCED: Only accumulate formal response content for retry context
                              // This prevents both finish tokens and thinking phase content from polluting retry context
                              if (parseResult && parseResult.cleanedText && !parseResult.isThought && formalResponseStarted) {
              ...
              switch (finishReason) {
                  case "STOP":
                      if (!strategist.isOutputtingFormalText) {
                          ...
                          // Trust the official STOP signal from upstream API when we have substantial content
                          const hasSubstantialContent = accumulatedText.trim().length > 50; // Minimum content threshold
                          ...
                          if (!isComplete) {
                              // NEW: Trust upstream STOP signal if we have reasonable content
                              if (hasSubstantialContent) {
                                  logInfo(`finishReason 'STOP' received with substantial content (${accumulatedText.trim().length} chars). Trusting upstream completion signal.`);
                                  logDebug(`Content preview: "${accumulatedText.trim().slice(-100)}"`);
                                  // Don't set interruptionReason, let the generation complete normally
                              } else {
                                  // Only retry if we truly have insufficient content
                                  ...
                                  logError(`Finish reason 'STOP' with insufficient content. Triggering retry.`);
              ...
              while (lookaheadLinesBuffer.length > 0) {
                  ...
                  if (forwardedTextLength + textLength <= safeTextLength) {
                      ...
                      if (lineInfo.isData) {
                          const parseResult = parseLineContent(lineInfo.line.replace(/\n\n$/, ''));
                          if (parseResult) {
                              // ENHANCED STATE UPDATE: Only accumulate formal response content for retry context
                              // This ensures retry attempts continue from the correct context without thinking phase content
                              if (parseResult.cleanedText && !parseResult.isThought && formalResponseStarted) {
                                  accumulatedText += parseResult.cleanedText;
                              }
            ...
            for (const lineInfo of lookaheadLinesBuffer) {
                if (lineInfo && lineInfo.line) {
                    ...
                    if (lineInfo.isData) {
                        ...
                        if (parseResult) {
                            // DUAL BEACON FINAL FIX: Only accumulate formal response content for retry context
                            // This prevents both markers and thinking content from polluting retry context
                            if (parseResult.cleanedText && !parseResult.isThought && formalResponseStarted) {
                                accumulatedText += parseResult.cleanedText;
                            }
